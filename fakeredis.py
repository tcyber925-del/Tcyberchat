"""Compatibility shim for `fakeredis` used in unit tests.

This module prefers the external `fakeredis` package when available and
falls back to a small in-repo shim implementing the subset of Redis
commands used by the test-suite (lists, hashes, sorted-sets and a
simple pipeline). The shim exposes `FakeRedis` and a `Redis.from_url`
compatibility helper to match how the codebase uses `redis.Redis.from_url`.
"""
...
try:
    # If an external fakeredis package is installed, delegate to it.
    import fakeredis as _real_fakeredis  # type: ignore

    FakeRedis = getattr(_real_fakeredis, "FakeRedis", getattr(_real_fakeredis, "FakeStrictRedis", None))

    class Redis:
        @staticmethod
        def from_url(url: str, decode_responses: bool = True):
            # Provide a FakeRedis instance regardless of the URL for tests.
            return FakeRedis()

except Exception:
    # Minimal fallback implementation used when fakeredis is not installed.
    ...
    class FakeRedis:
        ...
        def lrem(self, key: str, count: int, value: Any) -> int:
            ...
            if count > 0:
                ...
                self._lists[key] = newlst
                return removed
            # negative count: remove from tail
            if count < 0:
        ...
        # Sorted set operations (simple implementation)
    ...
    class Redis:
        """Compatibility helper: Redis.from_url(...) returns a FakeRedis"""

        @staticmethod
        def from_url(url: str, decode_responses: bool = True):
            return FakeRedis()
"""Lightweight fakeredis shim for unit tests in this workspace.

This file prefers the real `fakeredis` package when available; otherwise
it falls back to a minimal in-repo shim implementing the small subset of
Redis commands used by unit tests (lists, hashes, sorted-sets, pipeline).
"""
from __future__ import annotations
...
try:
    import fakeredis as _real_faker

    FakeRedis = _real_faker.FakeRedis
    ...
    try:
        # If the external `fakeredis` package is installed, delegate to it.
        import fakeredis as _real_fakeredis  # type: ignore

        FakeRedis = _real_fakeredis.FakeRedis

        class Redis:
            @staticmethod
            def from_url(url: str, decode_responses: bool = True):
                # Mirror the minimal compatibility API used in tests.
                return _real_fakeredis.FakeRedis()

    except Exception:
        # Fallback shim implementation used when `fakeredis` isn't installed.
        ...
        class FakeRedis:
            ...
            def lrem(self, key: str, count: int, value: Any) -> int:
                ...
                # negative count: remove from tail
                if count < 0:
                    # simplest: reverse and apply positive logic
                    rev = list(reversed(lst))
                    newrev = []
                    for v in rev:
                        if v == vs and removed < abs(count):
