...
class RAGCallbackHandler(BaseCallbackHandler):
    ...
    def on_chain_end(self, outputs, **kwargs):
        self.operations.append(
            {"operation": "chain_end", "timestamp": asyncio.get_event_loop().time()}
        )
    ...
    def on_retriever_end(self, documents, **kwargs):
        self.operations.append(
            {"operation": "retrieval_end", "documents_found": len(documents)}
        )
...
class RAGService:
    ...
    def __init__(self, persist_directory: str = "./chroma_db"):
        ...
        if LANGCHAIN_AVAILABLE:
            ...
            self.markdown_splitter = (
                None  # Markdown splitter not available without LangChain
            )
            ...
            try:
                ...
                self.vectorstore = create_vectorstore(
                    client=None, collection_name="documents", embedding=None
                )
            except Exception:
                self.vectorstore = None

    async def _get_ai_service(self, model_name: str | None = None):
    ...
    def _initialize_langchain_components(self):
        ...
        try:
            ...
            self.vectorstore = create_vectorstore(
                client=None, collection_name="documents", embedding=self.embeddings
            )
                ...
                class CombinedMarkdownSplitter:
                    ...
                    def split_text(self, text: str) -> list[str]:
                        ...
                        try:
                            ...
                            print(
                                f"Markdown header splitting failed, using recursive splitter: {e}"
                            )
                            return self.recursive_splitter.split_text(text)

                return CombinedMarkdownSplitter(
                    markdown_header_splitter, recursive_splitter
                )
    ...
    def _setup_advanced_retrievers(self):
        ...
        try:
            ...
            if similarity_retriever is None or mmr_retriever is None:
                print(
                    "Warning: One or more retrievers are None, skipping ensemble setup"
                )
                return
            ...
            try:
                ...
                if "takes no arguments" in str(te) or "positional" in str(te).lower():
                    ...
                    try:
                        ...
                        print(
                            f"Failed to create EnsembleRetriever with alternative method: {te}"
                        )
                        self.ensemble_retriever = None
                else:
                    raise
            ...
            if self.embeddings is not None:
                print(
                    f"DEBUG: Type of self.embeddings before EmbeddingsFilter: {type(self.embeddings)}"
                )
                from langchain_core.embeddings import Embeddings as LangchainEmbeddings

                print(
                    f"DEBUG: Is self.embeddings an instance of LangchainEmbeddings? {isinstance(self.embeddings, LangchainEmbeddings)}"
                )
                try:
                    ...
                    self.compression_retriever = DocumentCompressorPipeline(
                        base_retriever=base_retriever, transformers=[embeddings_filter]
                    )
                except TypeError:
        ...
        try:
            ...
            if isinstance(BaseLLM, type):
                ...
                class AIServiceLLM(BaseLLM):
                    ...
                    _ai_service_getter: Any | None = None
...
Answer:""")
            ...
            if self.ensemble_retriever is None:
                ...
                if base_retriever is None:
                    ...
                    return
                self.rag_chain = create_retrieval_chain(base_retriever, stuff_chain)
            else:
                self.rag_chain = create_retrieval_chain(
                    self.ensemble_retriever, stuff_chain
                )
...
Standalone question:""")
            ...
            if retriever_for_history is None:
                ...
                if retriever_for_history is None:
                    print(
                        "Warning: No retriever available for conversational chain setup"
                    )
                    return
...
Conversation History: {chat_history}
            ...
            self.conversational_chain = create_retrieval_chain(
                history_aware_retriever, conversational_stuff_chain
            )
    ...
    async def generate_rag_streaming_response(
        ...
        try:
            ...
            if model_name:
                ...
                try:
                    if (
                        hasattr(self, "rag_chain")
                        and self.rag_chain
                        and hasattr(self.rag_chain, "llm")
                    ):
                        if hasattr(self.rag_chain.llm, "_model_name"):
                            self.rag_chain.llm._model_name = model_name
                    if (
                        hasattr(self, "conversational_chain")
                        and self.conversational_chain
                        and hasattr(self.conversational_chain, "llm")
                    ):
            ...
            if use_web_search:
                try:
                    ...
                    ws_provider = (
                        getattr(web_search_service.primary_provider, "name", "unknown")
                        if web_search_service.primary_provider
                        else "none"
                    )
                    ...
                    # Enrich results with fetched content if enabled
                    web_results = await web_search_service.enrich_results(web_results)
                    ...
                    if web_results:
                        ...
                        web_context_parts = [
                            "=== Web Search Results (Most Recent Information) ==="
                        ]
                        for i, result in enumerate(web_results, 1):
                            ...
                            # Prefer enriched content over snippet if available
                            if result.content:
                                # Use fetched content (summarize to ~300 chars for context)
                                content_preview = (
                                    result.content[:300] + "..."
                                    if len(result.content) > 300
                                    else result.content
                                )
                                web_context_parts.append(f"Content: {content_preview}")
                                web_context_parts.append(
                            ...
                            if result.timestamp:
                                web_context_parts.append(
                                    f"Retrieved: {result.timestamp.strftime('%Y-%m-%d %H:%M:%S')}"
                                )
                        ...
                        # Log detailed provider information from web search service
                        ws_impl = getattr(web_search_service, "impl", "custom")
                        ws_provider = (
                            getattr(
                                web_search_service.primary_provider, "name", "unknown"
                            )
                            if web_search_service.primary_provider
                            else "none"
                        )
                        logger.info(
                            f"[RAG Web Search] impl={ws_impl}, provider={ws_provider}, "
                            f"results={len(web_results)}, query='{query[:50]}', time_sensitive={is_time_sensitive}"
                        )
                except Exception as e:
                    logger.warning(
                        f"Web search failed, continuing without web results: {e}",
                        exc_info=True,
                    )
            ...
            if conversational and chat_history and self.conversational_chain:
                ...
                if document_id:
                    ...
                    try:
                        ...
                        for doc_text, metadata, doc_id_vec in zip(
                            ...
                            doc_id_str = (
                                str(metadata.get("document_id", "")) if metadata else ""
                            )
                        ...
                        print(
                            f"Warning: Failed to filter conversational chain by document_id {document_id}: {e}"
                        )
                else:
                    # Use default retriever for conversational chain
                    base_retriever = self.ensemble_retriever or (
                ...
                if base_retriever:
                    ...
                    class WebSearchContextRetriever:
                        ...
                        async def aget_relevant_documents(self, query: str, k: int = 5):
                            ...
                            if hasattr(self.base_retriever, "aget_relevant_documents"):
                                docs = (
                                    await self.base_retriever.aget_relevant_documents(
                                        query, k
                                    )
                                )
                            else:
                    ...
                    wrapped_retriever = (
                        WebSearchContextRetriever(
                            base_retriever, web_search_context, is_time_sensitive
                        )
                ...
                async for chunk in self.conversational_chain.astream(
                    {
                        "input": query
                    },  # Use original query, web search is in context now
                    config=chain_config,
                ):
                    if "answer" in chunk:
                ...
                if document_id:
                    ...
                    try:
                        ...
                        print(
                            f"Warning: Failed to filter by document_id {document_id}: {e}"
                        )
                        search_kwargs = {"k": max_context_chunks}
                    ...
                    retriever = (
                        ...
                        or self.vectorstore.as_retriever(
                            search_kwargs={"k": max_context_chunks}
                        )
                    )
                ...
                class WebSearchContextRetriever:
                    ...
                    def __init__(
                        ...
                        self.is_time_sensitive = is_time_sensitive
                        logger.info(
                            f"WebSearchContextRetriever initialized with web_search_context length: {len(web_search_context) if web_search_context else 0}"
                        )
                    ...
                    def get_relevant_documents(self, query: str, k: int = 5):
                        ...
                        docs = self.base_retriever.get_relevant_documents(query, k)
                        logger.debug(
                            f"Base retriever returned {len(docs)} documents for query: '{query[:50]}'"
                        )
                        ...
                        if self.web_search_context:
                            ...
                            logger.info(
                                f"Adding web search document to context (time_sensitive={self.is_time_sensitive}, context_preview={self.web_search_context[:100]}...)"
                            )
                            # For time-sensitive queries, web search comes first
                            if self.is_time_sensitive:
                                result = [web_doc] + docs
                                logger.info(
                                    f"Web search document PREPENDED: total {len(result)} documents (web search is first)"
                                )
                                return result
                            else:
                                result = docs + [web_doc]
                                logger.info(
                                    f"Web search document APPENDED: total {len(result)} documents"
                                )
                                return result
                        return docs
                    ...
                    async def aget_relevant_documents(self, query: str, k: int = 5):
                        ...
                        if hasattr(self.base_retriever, "aget_relevant_documents"):
                            ...
                            docs = self.base_retriever.get_relevant_documents(query, k)
                        logger.debug(
                            f"Base retriever (async) returned {len(docs)} documents for query: '{query[:50]}'"
                        )
                        ...
                        if self.web_search_context:
                            ...
                            logger.info(
                                f"Adding web search document to context (async, time_sensitive={self.is_time_sensitive}, context_preview={self.web_search_context[:100]}...)"
                            )
                            # For time-sensitive queries, web search comes first
                            if self.is_time_sensitive:
                                result = [web_doc] + docs
                                logger.info(
                                    f"Web search document PREPENDED (async): total {len(result)} documents (web search is first)"
                                )
                                return result
                            else:
                                result = docs + [web_doc]
                                logger.info(
                                    f"Web search document APPENDED (async): total {len(result)} documents"
                                )
                                return result
                        return docs
                ...
                wrapped_retriever = (
                    WebSearchContextRetriever(
                        retriever, web_search_context, is_time_sensitive
                    )
                    if web_search_context
                    else retriever
                )
                ...
                async for chunk in self.rag_chain.astream(
                    ...
                    if "context" in chunk:
                        ...
                        source_docs = chunk.get("context", [])
                        logger.info(
                            f"Retrieved {len(source_docs)} documents from chain (including web search if present)"
                        )
                        ...
                        for i, doc in enumerate(source_docs):
                            if hasattr(doc, "metadata"):
                                ...
                                if (
                                    doc.metadata.get("source_type") == "web"
                                    or doc.metadata.get("source") == "web_search"
                                ):
                                    # Verify web search document is present
                                    logger.info(
                                        f"Web search document found in context at index {i}"
                                    )
...
Answer:"""
            ...
            yield {
                ...
                if web_search_used
                else 0,  # NEW: Count of web results
    ...
    async def add_document_with_chunking(
        self, document_id: str, full_text: str, metadata: dict[str, Any] | None = None
    ) -> bool:
        ...
        try:
            ...
            base_metadata = {"document_id": document_id, **(metadata or {})}
            ...
            if is_markdown and self.markdown_splitter:
                try:
                    ...
                    print(
                        f"Markdown splitter failed, falling back to regular splitter: {e}"
                    )
                    chunks = (
                        self.text_splitter.split_text(full_text)
            ...
            for i, chunk in enumerate(chunks):
                ...
                try:
                    ...
                    if LANGCHAIN_AVAILABLE and (
                        not isinstance(LangChainDocument, type)
                        or LangChainDocument is _Stub
                    ):
            ...
            try:
                print(
                    f"DEBUG: add_document_with_chunking - Adding {len(langchain_docs)} chunks for document_id={document_id}"
                )
                ...
                try:
                    ...
                    matching_count = 0
                    for metadata in all_docs.get("metadatas", []):
                        if metadata and str(metadata.get("document_id", "")) == str(
                            document_id
                        ):
                            matching_count += 1
                    print(
                        f"DEBUG: add_document_with_chunking - Verified: {matching_count} chunks with document_id={document_id} in vector store"
                    )
                except Exception as e:
            ...
            if document_id:
                ...
                for doc_text, metadata, doc_id in zip(
                    ...
                    if metadata.get("document_id") == document_id:
                        filtered_docs.append(
                            LangChainDocument(page_content=doc_text, metadata=metadata)
                        )
                ...
                if filtered_docs:
                    ...
                    try:
                        ...
                        print(
                            f"DEBUG: BM25 not available ({bm25_error}), using keyword matching on filtered documents"
                        )
    ...
    async def generate_rag_response(
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    for doc in session.query(DocModel).all():
                        ...
                        if (lower_q and lower_q in lower) or any(
                            k in lower for k in ["paris", "capital"]
                        ):
                            idx = (
                                lower.find(lower_q)
                                if lower_q and lower_q in lower
                                else -1
                            )
                            if idx == -1:
                                for kw in ["paris", "capital"]:
                                    if kw in lower:
                                        ...
                                        break
                            snippet = (
                                str(content)[max(0, idx - 50) : idx + 150]
                                if idx >= 0
            ...
            if conversational and chat_history and self.conversational_chain:
                ...
                adjusted_chunks = (
                    max_context_chunks // 2
                    if is_time_sensitive and web_search_context
                    else max_context_chunks
                )
                ...
                result["web_search_results_count"] = (
                    len(web_search_results) if web_search_used else 0
                )
                result["is_time_sensitive"] = (
                    is_time_sensitive  # NEW: Include time-sensitivity flag
                )
                return result
            elif self.rag_chain:
                # Use basic RAG chain
                            ...
                            if doc and getattr(doc, "content", None):
                                ...
                                if content:
                                    ...
                                    if query.lower() in content.lower() or any(
                                        term in content for term in ["paris", "capital"]
                                    ):
                    ...
                    try:
                        ...
                        for doc in session.query(DocModel).all():
                            ...
                            if not content:
                                continue
                            lower = str(content).lower()
                            if query.lower() in lower or any(
                                k in lower for k in ["paris", "capital"]
                            ):
                                # Return a short snippet containing the keyword
                                idx = (
                                    lower.find(query.lower())
                                    if query.lower() in lower
                                    else -1
                                )
                                if idx == -1:
                ...
                async def _get_ai_with_fallback(mname: str | None = None):
                    try:
                        getter = self._ai_service_getter
                        if getter is None:
                            # Lazy import to avoid hard dependency when running without full stack
                            from .ai_service import (
                                get_ai_service as getter,  # type: ignore
                            )
                        ...
                        # Last-resort fallback to default getter
                        try:
                            from .ai_service import (
                                get_ai_service as _fallback_getter,  # type: ignore
                            )

                            return await _fallback_getter(None)
                        except Exception:
                            raise
            ...
            async def _get_ai_with_fallback_local(mname: str | None = None):
                try:
                    getter = self._ai_service_getter
                    if getter is None:
    ...
    async def _generate_basic_rag_response(
        self,
        query: str,
        document_id: str | None = None,
        max_context_chunks: int = 3,
        ...
        if document_id:
            ...
            retriever = self.ensemble_retriever or self.vectorstore.as_retriever(
                search_kwargs={"k": max_context_chunks}
            )
    ...
    async def _generate_conversational_rag_response(
        self,
        query: str,
        document_id: str | None = None,
        max_context_chunks: int = 3,
            ...
            return {
                ...
                "memory_buffer_size": (
                    ...
                    if getattr(self.conversation_memory, "k", None) is not None
                        ...
                        getattr(self.conversation_memory, "memory", None), "k", None
                    )
                ),
            }
        ...
        return {
            ...
            "retrieval_stats": {
                "total_retrievals": len(
                    [
                        op
                        for op in self.callback_handler.operations
                        if op["operation"] == "retrieval_end"
                    ]
                ),
                "avg_documents_found": sum(
                    op.get("documents_found", 0)
                    for op in self.callback_handler.operations
                    if op["operation"] == "retrieval_end"
                )
                / max(
            ...
            "chain_stats": {
                "total_chains": len(
                    [
                        op
                        for op in self.callback_handler.operations
                        if op["operation"] == "chain_end"
                    ]
                )
            },
        }

    def configure_retrieval_strategy(
        self,
        ...
        try:
            if strategy == "ensemble":
                ...
                try:
                    ...
                    if (
                        "takes no arguments" in str(te)
                        or "positional" in str(te).lower()
                    ):
                        print(
                            f"Warning: EnsembleRetriever API issue, using fallback: {te}"
                        )
    ...
    async def hybrid_search(
        ...
        try:
            ...
            for doc_text, metadata in zip(
                all_docs["documents"], all_docs["metadatas"], strict=False
            ):
                    ...
                    bm25_results.append(
                        {
                            ...
                            "metadata": doc.metadata,
                            "score": 1.0
                            - (i * 0.1),  # BM25 doesn't provide scores directly
                            "document_id": doc.metadata.get("document_id"),
                            "chunk_index": doc.metadata.get("chunk_index"),
                            "search_type": "keyword",
                        }
    ...
    def get_langchain_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "observability": {
                ...
                "operations_tracked": len(self.callback_handler.operations)
...
if not LANGCHAIN_AVAILABLE:
    ...
    class RAGService:
        def __init__(self, persist_directory: str = "./chroma_db"):
            ...
            self.vectorstore = create_vectorstore(
                client=None, collection_name="documents"
            )
            self.embeddings = None
            self.text_splitter = create_splitter(chunk_size=1000, chunk_overlap=200)
            self.markdown_splitter = (
                None  # Markdown splitter not available in fallback mode
            )
            self.conversation_memory = create_memory(k=5)
            self.callback_handler = None
