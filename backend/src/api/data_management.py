...
async def export_data(
    ...
    try:
        ...
        if request:
            ...
            if start and end:
                ...
                if end_dt < start_dt:
                    ...
                    raise HTTPException(
                        status_code=422, detail="Invalid date range: end before start"
                    )
        ...
        for conv in conversations:
            ...
            if (
                start_dt
                and isinstance(conv_started, datetime)
                and conv_started < start_dt
            ):
                continue
        ...
        for doc in documents:
            ...
            if (
                start_dt
                and isinstance(doc_uploaded, datetime)
                and doc_uploaded < start_dt
            ):
                continue
            ...
            if include_documents:
                for doc in documents:
                    ...
                    if doc_path and Path(str(doc_path)).exists():
                        ...
                        zip_file.write(
                            str(doc_path),
                            f"documents/{getattr(doc, 'filename', 'file')}",
                        )
...
async def import_data(
    file: UploadFile = File(...), overwrite: bool = False, db: Session = Depends(get_db)
) -> dict:
    """
    ...
    try:
        ...
        with zipfile.ZipFile(zip_buffer, "r") as zip_file:
            ...
            if "data.json" not in zip_file.namelist():
                raise HTTPException(
                    status_code=400, detail="Invalid export file: missing data.json"
                )
            ...
            for conv_data in import_data.get("conversations", []):
                try:
                    ...
                    existing = (
                        ...
                        .filter(Conversation.id == conv_data["id"])
                        .first()
                    )
            ...
            for msg_data in import_data.get("messages", []):
                try:
                    ...
                    existing = (
                        db.query(Message).filter(Message.id == msg_data["id"]).first()
                    )
            ...
            for doc_data in import_data.get("documents", []):
                try:
                    ...
                    existing = (
                        db.query(Document).filter(Document.id == doc_data["id"]).first()
                    )
            ...
            for summary_data in import_data.get("summaries", []):
                try:
                    ...
                    existing = (
                        db.query(Summary)
                        .filter(Summary.id == summary_data["id"])
                        .first()
                    )
