...
def test_document_id_db_fallback_returns_snippet(monkeypatch):
    ...
    doc = Document(
        ...
        content="The capital of France is Paris. It is beautiful.",
    )
    session.add(doc)
    session.commit()
    session.refresh(doc)
    ...
    payload = {"message": "What is the capital of France?", "documentId": str(doc.id)}
...
def test_no_document_rag_global_search(monkeypatch):
    ...
    class MockRAG:
        async def generate_rag_response(self, query, document_id=None, model_name=None):
            return {
                "response": "This is an answer from the mocked RAG.",
                "citations": [{"docId": "mockdoc", "snippet": "mock snippet"}],
            }
