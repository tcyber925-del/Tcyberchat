...
def test_rag_service_fallback_flow(tmp_path):
    ...
    svc.vectorstore = _FallbackVectorStore(client=None, collection_name="test_rag_service")
    ...
    ok = asyncio.get_event_loop().run_until_complete(
        svc.add_document_with_chunking(document_id=doc_id, full_text=text, metadata={"title": "test"})
    )
    assert ok is True

    # Search for relevant chunks
    results = asyncio.get_event_loop().run_until_complete(
        svc.search_relevant_chunks(query="What is the capital of France?", document_id=doc_id, limit=3)
    )
    ...
    out = asyncio.get_event_loop().run_until_complete(
        svc.generate_rag_response(query="What is the capital of France?", document_id=doc_id)
    )
    assert isinstance(out, dict)
    assert "response" in out
    # Either RAG enabled with citation or fallback AI response exists
    assert (out.get("citations") and len(out.get("citations")) >= 0) or out.get("rag_enabled") in (True, False)
