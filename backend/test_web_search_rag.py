...
async def test_web_search_service():
    ...
    try:
        ...
        print(f"✓ Web search service initialized")
        ...
        print(f"\nTesting query: '{query}'")
        results = await service.search(query, max_results=3, use_cache=False, force_fresh=True)
    ...
    try:
        ...
        async for chunk in rag_service.generate_rag_streaming_response(
            ...
            if "citations" in chunk:
                ...
                web_citations = [c for c in citations if c.get("source") == "web_search" or c.get("source_type") == "web"]
                ...
                if web_citations_count > 0:
                    ...
                    for cit in web_citations[:3]:
                        print(f"    • {cit.get('title', 'N/A')[:50]} - {cit.get('url', 'N/A')[:50]}")
                else:
                    print("  ⚠ No web search citations found - web search may not have been used")
        ...
        if any(keyword in response.lower() for keyword in ["2024", "2025", "recent", "latest", "new"]):
        ...
        if web_search_used or web_citations_count > 0:
            ...
            print("⚠ Web search was enabled but no web citations found - integration may need verification")
...
async def test_time_sensitive_detection():
    ...
    try:
        ...
        for query, expected_time_sensitive in test_queries:
            ...
            print(f"  {status} '{query[:40]}...' -> {is_time_sensitive} (expected {expected_time_sensitive})")
