...
async def run():
    ...
    for i, chunk in enumerate(chunks):
        meta={'document_id':'d_test', 'chunk_index':i, 'chunk_length':len(chunk), 'total_chunks':len(chunks)}
    ...
    try:
        resp = await r.generate_rag_response('What is the capital of France?', document_id='d_test')
    ...
    try:
        ...
        with open(os.path.join('backend','logs','debug_rag.log'), 'a', encoding='utf-8') as f:
